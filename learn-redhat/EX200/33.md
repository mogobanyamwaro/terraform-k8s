...
**Errors and usage messages go to stderr with `>&2`.** See `02.md`.
...
1. **`>&2` sends the message to stderr.** Errors and diagnostics belong there so they are not captured by a pipeline. See `02.md`.
...
The `/etc/passwd` fields, from `10.md`:
...
**Note the `< 65534` bound.** UID 65534 is `nobody`, which would otherwise appear in a "regular users" list. See `10.md`.
...
**Enable the `.timer`, not the `.service`.** And **`daemon-reload` after creating the units.** See `19.md` and `14.md`.
