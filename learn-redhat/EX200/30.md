...
**`mount -o remount,rw /` first.** In emergency mode `/` is read-only, so `vim` cannot save and people panic. See `15.md` and `16.md`.
...
The filesystem is mounted on first access and unmounted after the idle timeout. **Useful for slow or removable devices.** For NFS this is usually done with autofs instead — see `32.md`.
...
**A `mount` can also hide the context of the underlying directory.** After mounting, `ls -Zd /data` shows the mounted filesystem's root, not the original directory. See `27.md`.
...
**Use `sudo tee -a`.** And note `-a`; without it `tee` truncates `/etc/fstab`, which is a catastrophe. See `02.md`.
...
In emergency mode `/` is mounted read-only, so `vim /etc/fstab` cannot save. **Without this command you cannot repair a broken `/etc/fstab`.** See `16.md` and Task 17 below.
...
```bash
sudo parted /dev/sdb resizepart 1 2GiB     # 28.md
sudo partprobe /dev/sdb
sudo xfs_growfs /data
df -h /data
```
...
If the root filesystem needs checking, do it from rescue media or `rescue.target` — you cannot unmount `/` while running from it. See `15.md`.
...
Related recovery topics: `15.md` for targets and emergency mode, `16.md` for `rd.break` and boot interruption.
...
**`/var/log/journal` is very often the answer.** `journalctl --vacuum-size=200M` reclaims it immediately. See `18.md`.
...
**For NFS, autofs is the conventional and exam-expected answer** rather than `x-systemd.automount` — see `32.md`. This mechanism is worth knowing for local devices, particularly slow or removable ones.
