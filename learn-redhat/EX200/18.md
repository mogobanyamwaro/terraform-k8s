...
**The two to remember: `/var/log/messages` for general problems, `/var/log/secure` for authentication.** When SSH key authentication fails, the reason is in `/var/log/secure` (see `09.md`). When a service misbehaves, `/var/log/messages` or the journal.
...
**`/var/log/secure` is the authentication log.** It is where you look for SSH key permission problems (`09.md`), sudo denials, and failed logins. The journal has the same content, but `/var/log/secure` survives even if the journal was volatile.
