...
```bash
# Partition with type 82 / swap  (28.md)
sudo fdisk /dev/sdb
...
**Prefer a drop-in file in `/etc/sysctl.d/` over editing `/etc/sysctl.conf`**, the same reasoning as `/etc/sudoers.d/` in `11.md` and systemd drop-ins in `14.md`.
