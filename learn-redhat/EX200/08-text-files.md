...
For a root-owned file, remember the `tee` form from `02-redirection-pipes.md`:
...
The **quoted** delimiter `<<'EOF'` is what keeps `$releasever` literal. With an unquoted `<<EOF`, bash would expand `$releasever` to an empty string and silently break the repo definition. This is a real failure mode in `22-software-management-dnf.md`.
...
**Never edit `/etc/sudoers` directly with vim.** Use `visudo`, which validates before saving. A syntax error there can lock you out of `sudo` entirely. See `11-password-aging-sudo.md`.
