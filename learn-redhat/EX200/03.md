...
`-q` produces no output and only sets the exit status, which is exactly what `if` needs. This shows up in `33.md`.
...
`-q` gives you the exit status with no output. This exact pattern reappears in `33.md` for shell scripting and is how you write conditional checks against config files.
