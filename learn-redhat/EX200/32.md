...
If the name does not resolve, fix that first — see `25.md`:
...
**Use the same server name the task uses.** If it says `server2.lab.example.com`, use that, and make sure it resolves (`25.md`) — an unresolvable name in `/etc/fstab` is a boot failure.
...
| `Name or service not known` | **The server name does not resolve** (`25.md`) |
| `No route to host` | **Firewall on the server** |
...
The blocked process sits in uninterruptible sleep (state `D` — see `13.md`) and cannot be killed:
...
**`exportfs -rav` re-reads `/etc/exports` and applies the changes with no interruption to existing mounts.** Restarting `nfs-server` would work but would disconnect every current client — the same relationship as `firewall-cmd --reload` versus restarting firewalld, or `systemctl reload httpd` versus `restart` (`14.md`).
...
| --- | --- | --- |
| sudo | `/etc/sudoers.d/` | `11.md` |
| systemd units | `/etc/systemd/system/UNIT.d/` | `14.md` |
| sysctl | `/etc/sysctl.d/` | `31.md` |
| NetworkManager | `/etc/NetworkManager/conf.d/` | `25.md` |
...
**A name resolved by DNS may not be resolvable early in the boot.** An `/etc/hosts` entry always is — see `25.md`:
...
| `autoconnect no` on the interface | `nmcli con mod ... autoconnect yes` (`24.md`) |
...
sudo chmod 2775 /export/shared             # collaborative dir (12.md)
```
