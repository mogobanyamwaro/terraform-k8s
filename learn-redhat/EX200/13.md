...
So if a task says "ensure this runs after a reboot", the answer is **never** `nohup` or `systemd-run`. It is a **systemd unit** (`14.md`), a **cron job or timer** (`19.md`), or a service you `enable` (`14.md`).
...
To stop something permanently you must `systemctl disable --now` it, not kill it. That distinction is examinable and is covered in `14.md`.
