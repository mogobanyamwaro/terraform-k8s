...
Behind the scenes this is a symlink, consistent with `05.md`:
...
Remember from `19.md` that `%` must be escaped as `\%` inside a crontab.
...
If `server2.lab.example.com` does not resolve, add it to `/etc/hosts` (see `25.md`) or use its IP address.
