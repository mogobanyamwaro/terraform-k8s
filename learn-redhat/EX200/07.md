...
Fields 4-8 map directly onto `chage` flags, which is the point of knowing this. See `11.md`.
...
`rpm -qd autofs` lists the doc files that package installed, which is more reliable than guessing directory names. You will typically find sample `auto.master` and `auto.misc` files showing exactly the map syntax needed in `32.md`.
...
This chain — `man -k` to find it, main page to see the subcommands, subcommand page for `/EXAMPLE` — is the single most valuable documentation skill for the SELinux tasks in `27.md`.
...
`test` is a shell builtin. `man test` shows the external `/usr/bin/test`, which exists but is not what bash actually runs. `help test` documents the builtin, including the `-f`, `-d`, `-z`, `-n`, `-eq` operators you need in `33.md`.
...
`timedatectl list-timezones` is preferable because it lists only names that `timedatectl set-timezone` will actually accept. See `21.md`.
...
You get `lvm`, `pvcreate`, `pvs`, `pvdisplay`, `vgcreate`, `vgs`, `vgextend`, `lvcreate`, `lvs`, `lvextend`, `lvremove`, and more. Every LVM command needed in `29.md` is discoverable this way, which means an LVM task is never unrecoverable even if you blank on the flags.
...
Copying an existing `.repo` file and editing the `name`, `baseurl`, and `gpgkey` is far faster and less error-prone than typing one from memory. See `22.md`.
