...
**There it is.** `/web` is `700`, so the `apache` user cannot traverse it. See `12-special-permissions-acls.md` for why `namei -l` is the right tool.
...
- **After changing the root password via `rd.break`** — see `16-boot-interrupt-root-recovery.md`. This is the exam-critical case.
...
**Without it, `/etc/shadow` is left labelled `unlabeled_t`** and the system will not let anyone log in — you have fixed the password and broken the login. See `16-boot-interrupt-root-recovery.md`.
...
If you do end up with an unbootable system, add `enforcing=0` or `selinux=0` to the kernel command line at the GRUB prompt (see `17-bootloader.md`) to get in and repair it.
