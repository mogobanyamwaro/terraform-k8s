...
If the menu flashes past too fast, hold **`Shift`** during boot (BIOS) or press **`Esc`** repeatedly (UEFI). To make it stay longer for practice, see `17.md`.
...
Reboot and confirm the menu waits 10 seconds. Full detail in `17.md`.
...
See `18.md`. Making the journal persistent is itself a plausible exam task, and it makes every subsequent troubleshooting task easier.
...
**The temporary nature of GRUB edits is a feature.** You do not want a recovery argument baked into the boot configuration. If a task asks you to add a **permanent** kernel argument, that is `/etc/default/grub` plus `grub2-mkconfig`, or `grubby`. See `17.md`.
