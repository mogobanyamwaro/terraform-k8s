...
The repository is empty, so there is nothing to install, but **the remote-add, remote-ls, remote-modify, remote-delete, and persistence workflow is all exercisable** — which is what the objective actually asks for. Note the same `file:///` three-slash rule as in `22.md`.
...
- **`file:///` needs three slashes** for a local remote, same rule as in `22.md`.
