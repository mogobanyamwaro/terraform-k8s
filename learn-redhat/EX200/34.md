...
**Read the task carefully.** "As the user `alice`, run a container" means rootless, as alice. Anything about a system service usually means rootful, or rootless with lingering enabled — see `35.md`.
...
**Remember the firewall** (`26.md`). Publishing a port makes podman listen, but firewalld still blocks external access:
...
**`:Z` sets `container_file_t` on the host directory.** Without it, the host directory keeps a label the container process is not permitted to read, and the application inside fails with permission errors that look nothing like an SELinux problem. See `27.md`.
...
- **A container as a system service usually means rootful**, or rootless with lingering. See `35.md`.
...
**`chmod 777` does not help**, because this is a MAC denial, not a DAC one — both layers must allow. See `27.md`.
...
**`sysctl -w` alone would not persist** — the file in `/etc/sysctl.d/` is what survives a reboot. Same pattern as `31.md`.

**Solution D — forward with firewalld** (`26.md`):
...
**"No route to host" is the firewall** (`26.md`):
...
| Network path | `ping` from the other host | Networking (`24.md`) |

**`--permanent` and `--reload`, both.** Without `--permanent` the rule vanishes at the next reboot; without `--reload` it is not active now. This is the most-failed detail on the whole exam — see `26.md`.
...
**A podman container has no daemon to restart it. You need a systemd unit — that is `35.md`.**
...
| Container created | **The definition yes; it is NOT running** | **A systemd unit — `35.md`** |
...
**Any task that says "the container must start at boot" or "run as a service" requires a systemd unit — that is `35.md`, and it is where the marks are.**
...
- **A running container does NOT survive a reboot.** You need a systemd unit — see `35.md`.
