...
**Use `nofail`.** Without it, booting without the ISO attached drops you into emergency mode. See `16.md`.
...
**Memorise this package name.** `semanage` is required for every SELinux port-label and fcontext task in `27.md`, and it is not installed on a minimal system. A task that says "label port 8404 for httpd" is impossible without it, and `dnf provides` is how you would recover if you forgot the name.
...
`uname -r` is what is **running**; `rpm -q kernel` is what is **installed**, and there can be several. They differ immediately after a kernel update, before you reboot. See `17.md` for choosing which one boots.
